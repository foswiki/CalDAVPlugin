...
my %formats = (
    ...
    holidaylist => {
        ...
        range =>
          '   * $day $month $year - $eday $emonth $eyear - $name - $summary',
    },
);
...
sub CALDAV {
    ...
    if ($calendar) {
        ...
        unless ($calspec) {
            return
              "<span class='foswikiAlert'>No such calendar '$calendar'</span>";
        }
    }
    else {
        $calspec = $params;
    ...
    my $cal;
    eval { $cal = Cal::DAV->new( user => $user, pass => $pass, url => $url ); };
    unless ($cal) {
    ...
    foreach my $e ( @{ $cal->entries() } ) {
        ...
        if ( $event{recurrence} ) {
            ...
            my $span = DateTime::Span->from_datetimes(
                start => $event{start} || DateTime->from_epoch( epoch => time ),
